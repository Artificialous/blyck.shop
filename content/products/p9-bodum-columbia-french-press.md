---
title: Bodum Columbia French Press, doppelwandig, Edelstahl, 1,5 l / 12 Tassen
categoryKey: kaffee
shortDesc: Doppelwandiger Edelstahl statt Glas — hält den Kaffee laut Hersteller
  bis zu zwei Stunden heiß, ganz ohne Wärmeplatte. Spülmaschinenfest, für bis zu
  12 Tassen.
icon: p-drip
badge: ""
baseClicks: 1621
price: 149,90
oldPrice: ""
priceDate: 17.08.2026
amazonLink: https://www.amazon.de/Bodum-Kaffeebereiter-Doppelwandig-Edelstahl-Sp%C3%BClmaschinengeeignet/dp/B001AIDGR0?th=1&linkCode=ll2&tag=blyck21-21&linkId=b8447671ad76260bf8ceea6adafcb307&ref_=as_li_ss_tl
heroImage: assets/uploads/bodum-doppelwandiger-frenchpress_columbia_img1.jpg
heroAiBadge: false
promoAiBadge: false
images:
  - zoom: 110
    moveX: 0
    moveY: 0
    aiBadge: false
    src: assets/uploads/bodum-doppelwandiger-frenchpress_columbia_img1.jpg
  - zoom: 110
    moveX: 0
    moveY: -5
    aiBadge: true
    src: assets/uploads/bodum-doppelwandiger-frenchpress_columbia_img2.png
cardZoom: 140
cardMoveX: -5
cardMoveY: -5
cardAiBadge: false
hasDetailPage: true
lead: Die doppelwandige Edelstahlkonstruktion hält den Kaffee laut Hersteller
  bis zu zwei Stunden heiß, ohne Wärmeplatte und ohne die Zerbrechlichkeit einer
  Glaskanne. Der Kolben kombiniert ein Silikon- und ein Mesh-Sieb, filtert
  dadurch feiner und lässt weniger Kaffeesatz in der Tasse zurück. Griff und
  Deckel bleiben beim Eingießen kühl anfassbar, der Deckel selbst verhindert
  ungewolltes Verschütten beim Kippen. Fasst 1,5 Liter, reicht für rund 12
  Tassen, komplett spülmaschinenfest.
pros:
  - Doppelwandiger Edelstahl statt Glas — bruchsicher, hält den Kaffee laut
    Hersteller bis zu zwei Stunden heiß
  - Silikon-Mesh-Kombination im Kolben filtert feiner, weniger Kaffeesatz in der
    Tasse
  - Komplett spülmaschinenfest
  - Von Bodum, der Marke, die die French Press im Massenmarkt bekannt gemacht
    hat — breite Ersatzteilversorgung
cons:
  - Mit rund 150 € deutlich teurer als einfache French-Press-Modelle — selbst
    Bodums eigene Glas-Variante Chambord kostet nur einen Bruchteil, dafür ohne
    die doppelwandige Isolierung
  - Mit 1,5 Litern für größere Runden gedacht — für eine Tasse allein
    unpraktisch groß
  - Metallgehäuse lässt anders als Glas von außen nicht erkennen, wie viel
    Kaffee noch da ist
  - Die zwei Stunden Warmhaltezeit sind eine Herstellerangabe, kein unabhängig
    geprüfter Wert
order: 6
slug: bodum-columbia-french-press
permalink: false
published: true
tags: products
id: p9
---
